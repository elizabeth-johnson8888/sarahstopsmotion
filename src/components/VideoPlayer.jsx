 ...
 */
export function VideoPlayer({ videoClassName = "w-full max-w-5xl", videoSrc }) {
  ...
  return (
    <div className={videoClassName}>
      ...
      >
        ...
        {/* put a play/pause button in the middle of the video */}
        <div className={`absolute inset-0 z-20 flex items-center justify-center bg-black/50 pointer-events-none ${showControls ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
          <button
              onClick={togglePlay}
      ...
      {/* ── BELOW-VIDEO CONTROLS ── only shown outside fullscreen */}
      {!isFullscreen && (
        <div className="flex items-center gap-2 px-2 pt-2">

          <button onClick={skipBack} aria-label="Skip back 10 seconds"
            className="w-8 h-8 flex items-center justify-center opacity-75 hover:opacity-100 transition-opacity">
            <img src={ICONS.skipBack} alt="Skip back 10s" className="w-5 h-5 invert" />
          </button>

          <button onClick={togglePlay} aria-label={isPlaying ? "Pause" : "Play"}
            className="w-10 h-10 flex items-center justify-center opacity-90 hover:opacity-100 transition-opacity">
            <img src={isPlaying ? ICONS.pause : ICONS.play} alt={isPlaying ? "Pause" : "Play"} className="w-7 h-7 invert" />
          </button>

          <button onClick={skipForward} aria-label="Skip forward 10 seconds"
            className="w-8 h-8 flex items-center justify-center opacity-75 hover:opacity-100 transition-opacity">
            <img src={ICONS.skipForward} alt="Skip forward 10s" className="w-5 h-5 invert" />
          </button>
          ...
          <div className="flex items-center gap-1.5 group/vol">
            <button onClick={toggleMute} aria-label={isMuted ? "Unmute" : "Mute"}
              className="w-8 h-8 flex items-center justify-center opacity-75 hover:opacity-100 transition-opacity">
              <img src={isMuted ? ICONS.volumeOff : ICONS.volumeOn} alt={isMuted ? "Unmute" : "Mute"} className="w-5 h-5 invert" />
            </button>
            <input type="range" min={0} max={1} step={0.02}
              value={isMuted ? 0 : volume} onChange={handleVolumeChange}
              aria-label="Volume"
              className="w-0 group-hover/vol:w-20 transition-[width] duration-300 h-1 cursor-pointer accent-white appearance-none bg-white/25 rounded-full overflow-hidden"
            />
          </div>

          <button onClick={toggleFullscreen} aria-label="Enter fullscreen"
            className="w-8 h-8 flex items-center justify-center opacity-75 hover:opacity-100 transition-opacity ml-1">
            <img src={ICONS.fullscreen} alt="Fullscreen" className="w-5 h-5 invert" />
          </button>
